...
#define BUFSIZE 512
#define XFADE 16 // number of samples for crossfade
#define MAXSAMPLES BUFSIZE - XFADE // number of samples for crossfade
const float CROSSFADE_FACTOR = 0.5f;

const float SCALE_FACTOR = 0.9342492889f;// 440.f / 470.f;
const float THIRD = 5.f / 4.f;
const float FOURTH = 3.f / 4.f;
const float FIFTH = 3.f / 2.f;
const float PITCH_FACTOR = 1.f;
const long SAMPLE_FREQUENCY = 27778;// 13889 rounded from 1/7.2e-5 = 13888.88
const long SPEAKER_FREQUENCY = SAMPLE_FREQUENCY * SCALE_FACTOR * PITCH_FACTOR;

const long AMPLITUDE_LOW_THRESH = 1000;
const long AMPLITUDE_HIGH_THRESH = 4096;
...
const int BUFLEN = BUFSIZE;

bool isBufA = true;
int bufferA[BUFSIZE] = {0};
int bufferB[BUFSIZE] = {0};
int bufferX[BUFSIZE] = {0};
...
void TC5_Handler (void) {
  ...
  int amplitude;
  if (isBufA){
    amplitude = bufferB[outIdx];
  }
  else {
    amplitude = bufferA[outIdx];
  }
...
const int XFADE_DURATION = 4; // idek, just following chatgpt
int crossfadeCount = 0;
void crossfade(){
  // We read into one buffer, and read out of the other

  // Assume bufferB is already crossfaded and we just
  // stopped reading from bufferA for the speaker

  // bufferB will be read from, and bufferA will be
  // populated with new mic values. We should
  // crossfade bufferA with the constant bufferB
  // so that when A is finished, the speaker
  // can go onto reading from the crossfaded
  // bufferA.

  // NOTE: This function assumes inBuf[i] is 
  // already populated with most recent micval
  // otherwise this is pointless

  // isBufA means we read INTO bufferA
  // and that we read FROM bufferB
  // Denote the buffer we're crossfading
  // as the inBuf and the buffer we're
  // referencing as the refBuf
  int *inBuf = isBufA ? bufferA : bufferB;
  int *refBuf = isBufA ? bufferB : bufferA;
  float p = ((float)crossfadeCount) / XFADE_DURATION;

  // Fading from refBuf to the inBuf, therefore refBuf should dominate
  // at first and linearly back off
  for (int i=0; i < BUFSIZE; i++){
    int curIdx = (inIdx + i) % BUFSIZE;
    inBuf[curIdx] = (int)((refBuf[curIdx] * (1.f - p)) + (inBuf[curIdx] * p));
  }
...
void loop() {
  ...
  int amplitude = readADCSync();

  // Maintain two buffers that will be read into based on
  // whether isBufA is toggled
  if (isBufA){
    bufferA[inIdx] = amplitude;  
  }
  else {
    bufferB[inIdx] = amplitude;  
  }
  inIdx = (inIdx + 1) % BUFSIZE;


  // Toggle isBufA if index is equal to 0
  if (inIdx == 0){
    isBufA = !isBufA;
  }
